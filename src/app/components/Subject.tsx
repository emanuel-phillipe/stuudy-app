...
function Subject({subjectName, maxPoints, currentGrade, components, handleClick}:ISubjectProps) {
  const [hovered, setHovered] = useState(false)

  const subjectDescription = components.length > 1 ? components.join(", ") : "Componente Único"

  return (
    <Link href={'/subject'} onClick={handleClick} onMouseEnter={() => {setHovered(true)}} onMouseLeave={() => {setHovered(false)}} className="py-2 px-3 border-[0.7px] items-center flex justify-between border-zinc-200 hover:border-zinc-400 transition-all cursor-pointer rounded-lg w-full">
      <div>
